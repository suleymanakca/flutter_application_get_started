class UserManagement<T extends AdminUser> {
  ...
  int calculateMoneys(List<GenericUser> users) {
    ...
    // ideali alttaki, high order functions
    final _money = users.fold(initialValue, (previousValue, element) => previousValue + element.money);
    return _money;
  }

  Iterable<VBModels<R>>? showNames<R>(List<GenericUser> users) {
    if (R == String) {
      final names = users.map((e) => VBModels<R>(e.name.split(' ').toList().cast<R>()));
      return names;
    }
    return null;
  }
